"""API routers for Supervisor Agent — incidents, alerts, approvals, audit, SSE."""
...
async def delete_incident(
    ...
    await RedisService(redis).publish_event(
        "incident_deleted", {"incident_id": incident_id}
    )
    return {"status": "ok"}
...
@router.get("/health")
async def health():
    return {"status": "ok", "service": "supervisor_ai_agent"}
...
async def get_incident_detail(
    ...
    # Build LLM analysis — supervisor format uses root_cause dict
    llm_analysis = None
    if incident.ai_analysis_json:
        ai = incident.ai_analysis_json
        rc = ai.get("root_cause", {})

        # Supervisor LLM format: root_cause is a dict with category, summary_vi
        if isinstance(rc, dict) and "category" in rc:
            category = rc.get("category", "UNKNOWN")
            summary = rc.get("summary_vi", "")
            confidence = rc.get("confidence", 0)
            escalate = ai.get("escalate", False)
            escalate_reason = ai.get("escalate_reason", "")
            ...
            # Fallback: host-resource format
            llm_analysis = LLMAnalysis(
                summary=ai.get("summary", incident.summary or ""),
                root_causes=[
                    RootCauseItem(**rc_item)
                    for rc_item in ai.get("root_causes", [])
                ],
    ...
    return IncidentDetail(
        incident={
            ...
            "domain_type": incident.domain_type,
...
async def create_approval(
    ...
    if req.decision == "approved":
        ...
        if req.selected_commands is not None:
            ...
            logger.info(
                f"[APPROVAL] Operator selected {len(filtered)}/{len(all_cmds)} commands"
            )
        ...
        await repo.save_audit(
            ...
            action="approve",
            details={"option_id": req.action_proposal_id, "title": option.title},
        )
        await db.commit()

        host = (
            incident.instance.split(":")[0]
            if ":" in incident.instance
            else incident.instance
        )
        asyncio.create_task(
            _handle_execution(incident.id, req.action_proposal_id, host, redis_svc)
        )
        ...
        await db.commit()
        await redis_svc.publish_event(
            "action_canceled", {"incident_id": incident.id}
        )
...
async def suppress_incident(incident_id: str, db: AsyncSession = Depends(get_db)):
    redis = await get_redis()
    redis_svc = RedisService(redis)
    repo = IncidentRepository(db)
    ...
    if incident.canonical_root_cause:
        await repo.save_pattern(
            ...
            description=f"Operator noise: {incident.root_cause or incident.alert_name}",
            created_by="operator",
        )
...
async def unsuppress_incident(incident_id: str, db: AsyncSession = Depends(get_db)):
    redis = await get_redis()
    redis_svc = RedisService(redis)
    repo = IncidentRepository(db)
    ...
    new_status = (
        IncidentStatus.ACTION_PROPOSED if incident.root_cause else IncidentStatus.NEW
    )
    await repo.update_incident(incident_id, status=new_status, final_status=None)
    ...
    await redis_svc.publish_event(
        "incident_unsuppressed", {"incident_id": incident_id}
    )
    return {"status": "ok"}
...
async def monitor_incident(
    ...
    await redis.setex(f"agent:dedup:{fingerprint}", duration * 60, incident_id)

    await repo.update_incident(
        incident_id,
        status="monitoring",
        summary=f"Theo dõi {duration} phút.",
    )
    await repo.save_audit(
    ...
    await repo.save_incident_event(
        incident_id, "monitoring_started", {"duration_minutes": duration}
    )
    await db.commit()
...
async def skip_llm(incident_id: str, db: AsyncSession = Depends(get_db)):
    redis = await get_redis()
    redis_svc = RedisService(redis)
    repo = IncidentRepository(db)
...
async def _handle_execution(
    incident_id: str, option_id: str, host: str, redis_svc: RedisService
):
    ...
    try:
        ...
        async with factory() as db:
            ...
            result = await exec_svc.execute_approved_action(
                incident_id, option_id, host
            )
