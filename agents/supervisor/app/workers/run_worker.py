...
print(f"[WORKER] Starting supervisor worker process...")
...
async def main():
    ...
    print(
        f"[WORKER] DB: "
        f"{settings.database_url.split('@')[1] if '@' in settings.database_url else '?'}"
    )
    ...
    try:
        ...
        qlen = await redis.llen("agent:queue:supervisor")
