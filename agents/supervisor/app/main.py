...
async def lifespan(app: FastAPI):
    ...
    if settings.orchestrator_url:
        # Orchestrator mode: wait for tables to be ready
        logger.info(
            f"Orchestrator mode: waiting for DB tables from {settings.orchestrator_url}..."
        )
...
async def root():
    ...
    return (
        HTMLResponse(f.read_text())
        if f.exists()
        else HTMLResponse("<h1>Supervisor Agent</h1>")
    )
